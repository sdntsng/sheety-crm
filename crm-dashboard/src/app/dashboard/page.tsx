          ...
          <div className="pl-12">
            ...
            <div className="space-y-4">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="flex justify-between items-end border-b border-[var(--border-pencil)] border-dashed pb-1"
                >
...
function DashboardPageContent() {
  ...
  if (status === "loading" || checkingStorage) {
    return <DashboardSkeleton subtitle="Initializing Dashboard…" />;
  }
  ...
  if (loading) {
    return <DashboardSkeleton subtitle="Fetching your data…" />;
  }
  ...
  const visiblePipeline = Object.entries(data.pipeline_by_stage).filter(
    ([stage]) => !hiddenStages.includes(stage),
  );

  const visibleLeads = Object.entries(data.leads_by_status).filter(
    ([status]) => !hiddenStatuses.includes(status),
  );

  const activeLeads = visibleLeads.reduce((sum, [, count]) => sum + count, 0);
  const activeOpportunities = visiblePipeline.reduce(
    (sum, [, val]) => sum + val.count,
    0,
  );
  const pipelineValue = visiblePipeline.reduce(
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              ([stage, stageData]: [
                ...
                return (
                  <div key={stage} className="flex items-center gap-4 group">
                    <div className="w-32 font-mono text-xs font-bold text-[var(--text-secondary)] uppercase">
                      {stage}
                    </div>
                    ...
                    <div className="w-24 text-right font-sans font-bold">
                      {formatCurrency(stageData.total_value)}
                    </div>
                    <div className="w-8 text-center font-mono text-xs bg-[var(--bg-paper)] border border-[var(--border-pencil)] rounded">
                      {stageData.count}
                    </div>
                  </div>
                );
        ...
        <div className="paper-card p-6 bg-[var(--bg-paper)] border-2 border-[var(--border-pencil)] relative">
          ...
          <div className="pl-12">
            <h2 className="font-sans font-bold text-xl mb-6 underline decoration-[var(--accent-blue)] decoration-2 underline-offset-4">
              Lead Status
            </h2>
            ...
            <div className="mt-8 pt-4 border-t-2 border-[var(--border-ink)]">
              <Link
                href="/leads"
                className="font-mono text-xs uppercase font-bold hover:underline flex items-center justify-between group"
              >
