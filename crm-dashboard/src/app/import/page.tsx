...
export default function ImportPage() {
  ...
  const isRequiredMapped = (field: string) =>
    mappings.some((m) => m.crm_field === field);
  const missingRequired =
    !isRequiredMapped("company_name") || !isRequiredMapped("contact_name");
  ...
  const handlePreview = async () => {
    ...
    if (missingRequired) {
      setError(
        "Please map both Company Name and Contact Name (required fields)",
      );
      return;
    }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {step === "upload" && (
        <div className="bg-white border-2 border-[var(--border-ink)] p-8 shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
          <h2 className="text-2xl font-sans font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2">
            <span className="text-[var(--accent-blue)]">■</span> Step 1: Upload
            CSV File
          </h2>
          ...
          <div
            className={`border-2 border-dashed ${importing ? "border-[var(--accent-blue)] bg-[var(--bg-hover)]" : "border-[var(--border-pencil)]"} p-12 text-center hover:border-[var(--accent-blue)] transition-colors relative`}
          >
            {importing ? (
              <div className="py-8">
                <div className="text-4xl animate-bounce mb-4">⏳</div>
                <p className="font-sans font-bold text-lg text-[var(--text-primary)]">
                  Parsing CSV File...
                </p>
                <p className="font-mono text-xs text-[var(--text-secondary)] mt-2 uppercase">
                  Please wait
                </p>
              </div>
            ) : (
      ...
      {step === "mapping" && uploadData && (
        <div className="bg-white border-2 border-[var(--border-ink)] p-8 shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
          <h2 className="text-2xl font-sans font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2">
            <span className="text-[var(--accent-blue)]">■</span> Step 2: Map
            Columns
          </h2>
          <p className="font-mono text-xs text-[var(--text-secondary)] mb-6 uppercase tracking-wider">
            Match CSV columns to CRM fields • {uploadData.total_rows} rows to
            import
          </p>
          ...
          {missingRequired && (
            <div className="mb-4 p-3 bg-[var(--accent-yellow)]/10 border border-[var(--accent-yellow)] text-[var(--accent-yellow)] text-xs font-mono">
              REQUIRED: Please map at least "Company Name" and "Contact Name" to
              proceed.
            </div>
          )}
      ...
      {step === "preview" && (
        <div className="bg-white border-2 border-[var(--border-ink)] p-8 shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
          <h2 className="text-2xl font-sans font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2">
            <span className="text-[var(--accent-blue)]">■</span> Step 3: Preview
            Data
          </h2>
          <p className="font-mono text-xs text-[var(--text-secondary)] mb-6 uppercase tracking-wider">
                ...
                {previewData.map((row, idx) => (
                  ...
                  >
                    {Object.values(row).map((value, colIdx) => (
                      <td key={colIdx} className="px-4 py-3 font-sans text-sm">
                        {value || "-"}
                      </td>
                    ))}
                  </tr>
          ...
          <div className="flex gap-4">
            ...
            >
              {importing
                ? "Importing..."
                : `Import ${uploadData?.total_rows || 0} Leads`}
            </button>
          </div>
        </div>
      )}
      ...
      {step === "complete" && importResult && (
        <div className="bg-white border-2 border-[var(--border-ink)] p-8 shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
          <h2 className="text-2xl font-sans font-bold text-[var(--text-primary)] mb-4 flex items-center gap-2">
            <span className="text-[var(--accent-green)]">✓</span> Import
            Complete
          </h2>
