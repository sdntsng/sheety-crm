...
export default function SearchBar() {
  ...
  const allResults = results
    ? [
        ...
        ...results.results.opportunities.map((o) => ({
          ...o,
              ...
              {results.results.leads.length > 0 && (
                <div className="mb-2">
                  ...
                  {results.results.leads.map((lead, idx) => (
                    <button
                      key={lead.lead_id}
                      className={`w-full px-2 py-2 text-left hover:bg-[var(--bg-hover)] transition-colors flex items-center gap-3 group ${
                        selectedIndex === idx ? "bg-[var(--bg-hover)]" : ""
                      }`}
                      onClick={() =>
                        navigateToResult({ ...lead, _type: "lead" })
                      }
                    >
                  ...
                  {results.results.opportunities.map((opp, idx) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          navigateToResult({ ...opp, _type: "opportunity" })
                        }
                      >
