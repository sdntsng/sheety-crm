...
class MockSheetManager:
    ...
    def _load_data(self) -> dict:
        ...
        if not os.path.exists(self.file_path):
            console.print(f"[yellow]Mock data file not found at {self.file_path}. Creating new one.[/yellow]")
            return {"Sales Pipeline 2026": {}}
        try:
            with open(self.file_path, "r") as f:
                return json.load(f)
        except json.JSONDecodeError:
            console.print(f"[red]Error decoding {self.file_path}. Starting empty.[/red]")
            return {"Sales Pipeline 2026": {}}
    ...
    def update_cell(self, sheet_name: str, cell_address: str, value: str, worksheet_name: str = "Sheet1"):
        ...
        console.print(f"[yellow]MOCK: Single cell update {cell_address} not supported yet. Use row updates.[/yellow]")
    ...
    def update(self, range_name, values):
         ...
         if range_name.startswith('A'):
             try:
                 ...
                 if len(values) == 1:
                     self.manager.update_row(self.sheet_name, row_idx, values[0], self.title)
             except:
                 pass
